	...
	if ([self hasSecondWord]) {
        ...
        if (itemToTake == nil ) {
            ...
            if ([itemToTake weight] + [player currentWeight] > [player maxWeight] ) {
                ...
                [player outputMessage:[NSString stringWithFormat:@"\nYou try to lift the %@, but it does not budge.  Perhaps you are carrying too much.", secondWord]];
                ...
                if ([player hasTakenItem] == false ) {
                    [player outputMessage: [NSString stringWithFormat:@"\nYou search for something to carry the %@ in but find nothing.  Your pants, once fine, are torn, and seem to have been made without pockets.  You take off your coat and, with a little work, fashion a crude backpack of sorts, hoping that the %@ will remain secure.", secondWord, secondWord]];
                    [player setHasTakenItem: true];
                } else {                    
                    [player outputMessage: [NSString stringWithFormat:@"\nYou place the %@ in your backpack.", secondWord]];
                }
                
                if ([player maxWeight] - [player currentWeight] < 10) {
                    //the player is close to being over-encumbered
                    [player outputMessage:@"\nYour backpack rest uncomfortably on your back.  You aren't sure how much more you can carry."];
                }
            }
        }
        [itemToTake release];
